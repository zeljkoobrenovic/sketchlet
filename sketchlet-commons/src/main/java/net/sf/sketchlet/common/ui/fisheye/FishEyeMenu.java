...
class FishEyeWindow extends JWindow implements MouseListener, MouseMotionListener, KeyListener {
    ...
    public FishEyeWindow(FishEyeMenu menu) {
        ...
        if (up == null) {
            up = UtilContext.getInstance().getImageIconFromResources("resources/arrow-up.gif").getImage();
            down = UtilContext.getInstance().getImageIconFromResources("resources/arrow-down.gif").getImage();
        }
    }
    ...
    public void mouseMoved(MouseEvent e) {
        ...
        if (e.getX() < getSize().width / 2) {
            ...
            if (!focusLock) {
                // Just entered focus lock mode, so updateVariable look up tables, and calculate focus item position
