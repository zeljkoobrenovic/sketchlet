...
public class Perspective {
    ...
    public void doPerspectiveAndDrawImage(Graphics2D g, BufferedImage buffer) {
        ...
        if (region.parent != null) {
            region.parent.page.calculateHorizonPoint();
            h_x1 = region.parent.page.perspective_horizont_x1;
            h_x2 = region.parent.page.perspective_horizont_x2;
            h_y1 = region.parent.page.perspective_horizont_y;
            h_y2 = region.parent.page.perspective_horizont_y;
        } else {
            h_x1 = 500;
            h_x2 = 1000;
            h_y1 = 500.0;
        ...
        if (!region.processText(region.strRotation3DHorizontal).isEmpty()) {
            ...
            String type = region.parent.page.getPropertyValue("perspective type");
        ...
        if (p_x0 != 0.0 || p_y0 != 0.0 || p_x1 != 1.0 || p_y1 != 0.0 || p_x2 != 1.0 || p_y2 != 1.0 || p_x3 != 0.0 || p_y3 != 1.0) {
            ...
            min_x = (int) Math.min(p_x0 * w, p_x1 * w);
            min_x = (int) Math.min(min_x, p_x2 * w);
            min_x = (int) Math.min(min_x, p_x3 * w);

            min_y = (int) Math.min(p_y0 * h, p_y1 * h);
            min_y = (int) Math.min(min_y, p_y2 * h);
            min_y = (int) Math.min(min_y, p_y3 * h);
            ...
            buffer2 = Workspace.createCompatibleImage(Math.abs(max_x - min_x), Math.abs(max_y - min_y), buffer2);
            ...
            g.drawImage(buffer, region.getX1() + min_x, region.getY1() + min_y, null);
        } else {
