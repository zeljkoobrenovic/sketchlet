...
public class TextDrawingLayer extends DrawingLayer {
    ...
    public static Hashtable<String, Font> fontCache = new Hashtable<String, Font>();
    public Vector<String> textFileLines = new Vector<String>();
    Font font = null;
    LineMetrics metrics;
    float xText;
    float yText;
    // public VolatileImage cachedImage = null;
    public BufferedImage cachedImage = null;
    Vector<String> prevTextLines = new Vector<String>();
    int regionWidth = 0;
    int regionHeight = 0;
    String strHAlign = "";
    String strVAlign = "";
    String strTextParams = "";
    String strFont = "";
    ...
    public boolean checkShouldDraw(Vector<String> textLines) {
        ...
        strHAlign = region.processText(region.strHAlign);
        strVAlign = region.processText(region.strVAlign);
        strTextParams = region.bTrimText + ";" + region.strCharactersPerLine + ";" + region.strMaxNumLines;
    ...
    public boolean isTextChanged(Vector<String> textLines) {
        ...
        if (!strTextParams.equals(region.bTrimText + ";" + region.strCharactersPerLine + ";" + region.strMaxNumLines)) {
            return true;
        }
    ...
    public void draw(Graphics2D g2, Component component, boolean bPlayback) {
        ...
        if (region.strText.trim().length() == 0 && strText.isEmpty() /*&& strTextFile.isEmpty()*/) {
            return;
        }
        ...
        if (region.strText.trim().length() > 0) {
            String strTextArea = region.processText(region.strText);
    ...
    public void drawTextLines(Component component, Vector<String> textLines, String strText, boolean bPlayback) {
        try {
            ...
            if (region.bWrapText && !region.strCharactersPerLine.equals("")) {
    ...
    public Vector<String> wrapLines(Vector<String> textLines) {
        ...
        try {
            ...
            int maxLines = 0;
            if (!region.strMaxNumLines.trim().equals("")) {
                maxLines = Integer.parseInt(region.strMaxNumLines);
            }
    ...
    public static Font getFont(String strName) {
        Font font = fontCache.get(strName);
        if (font == null) {
            if (strName.length() > 0) {
                font = new Font(strName, Font.PLAIN, 12);
                fontCache.put(strName, font);
                return font;
            }
        } else {
            return font;
