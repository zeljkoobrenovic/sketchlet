...
public class ActiveRegionOverlapHandler {
    ...
    Vector<ActiveRegion> overlappingRegions = new Vector<ActiveRegion>();
    Vector<ActiveRegion> containedRegions1 = new Vector<ActiveRegion>();
    Vector<ActiveRegion> containedRegions2 = new Vector<ActiveRegion>();
    Vector<ActiveRegion> containedRegions3 = new Vector<ActiveRegion>();
    ActiveRegion region;
    ...
    public void processInteractionEvents(boolean bPlayback, Vector<TimerThread> activeTimers, Vector<RunInterface> activeMacros) {
        if (!bPlayback) {
            return;
        }
        ...
        for (RegionOverlapEventMacro regionOverlapEventMacro : region.regionOverlapEventMacros) {
            ...
            try {
                ...
                if (regionId.equals("")) {
                    ...
                    for (ActiveRegion a : region.parent.regions) {
                        if (a != region) {
                    ...
                    try {
                        ...
                        if (index >= 0 && index < region.parent.regions.size()) {
                            ActiveRegion a = region.parent.regions.elementAt(index);
                            processIntersection(bPlayback, region, a, r1, event, regionOverlapEventMacro.getMacro(), activeTimers, activeMacros);
                        }
                    } catch (Exception e) {
                    }
    ...
    public void processIntersection(boolean bPlayback, ActiveRegion sourceRegion, ActiveRegion region, Area r1, String event, Macro macro, Vector<TimerThread> activeTimers, Vector<RunInterface> activeMacros) {
