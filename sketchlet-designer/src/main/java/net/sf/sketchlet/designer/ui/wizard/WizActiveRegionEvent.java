...
public class WizActiveRegionEvent {
    ...
    public static void showWizard(int type, final String strTitle) {
        ...
        final MyProducer mp = new MyProducer(SketchletEditor.editorPanel.currentPage.regions.selectedRegions == null ? null : SketchletEditor.editorPanel.currentPage.regions.selectedRegions.lastElement());
        ...
        r = new Runnable() {
            ...
            public void run() {
                ...
                if (SketchletEditor.editorPanel.timersTablePanel != null) {
...
class Result extends DeferredWizardResult {
    ...
    public void start(Map settings, ResultProgressHandle progress) {
        ...
        if (mp.actionPage.actionParamPage.action.equalsIgnoreCase("Region Animate")) {
            ...
            try {
                String strCommentPrefix = "On '" + SketchletEditor.editorPanel.currentPage.title + "' ";
    ...
    public void changeRegionProperties(Vector<String> comments) {
        ...
        if (mp.type == 1) {
            int index = SketchletEditor.editorPanel.currentPage.regions.regions.size() - SketchletEditor.editorPanel.currentPage.regions.regions.indexOf(mp.region);
            comments.add("On '" + mp.eventPage.mouseEvent.getSelectedItem().toString().toLowerCase() + "' in Region '" + (index) + "' do:");
        } else if (mp.type == 2) {
            comments.add("When Region '" + mp.eventPage.regionsCombo.getSelectedItem() + "' " + mp.eventPage.interactionEvent.getSelectedItem() + " Region '" + (SketchletEditor.editorPanel.currentPage.regions.regions.size() - SketchletEditor.editorPanel.currentPage.regions.regions.indexOf(mp.region)) + "' do:");
        } else if (mp.type == 3) {
            comments.add("On '" + SketchletEditor.editorPanel.currentPage.title + "' entry");
        } else if (mp.type == 5) {
            comments.add("On '" + SketchletEditor.editorPanel.currentPage.title + "' variable update");
        } else if (mp.type == 6) {
            comments.add("On '" + SketchletEditor.editorPanel.currentPage.title + "' keyboard event");
        } else if (mp.type == 4) {
            comments.add("On '" + SketchletEditor.editorPanel.currentPage.title + "' exit");
        }
    ...
    public void animateRegion(Timer timer, Vector<String> comments) {
        ...
        comments.add("Timer '" + timer.name + "' is configured to:");
        for (int p = 0; p < transformations.length; p++) {
            JCheckBox checkBox = (JCheckBox) transformations[p][0];
        ...
        if (mp.type == 1) {
            int index = SketchletEditor.editorPanel.currentPage.regions.regions.size() - SketchletEditor.editorPanel.currentPage.regions.regions.indexOf(mp.region);
            comments.add("On '" + mp.eventPage.mouseEvent.getSelectedItem().toString().toLowerCase() + "' in Region '" + (index) + "' do:");
            setMouseEvent("Start timer", timer.name, "", comments);
        } else if (mp.type == 2) {
            comments.add("When Region '" + mp.eventPage.regionsCombo.getSelectedItem() + "' " + mp.eventPage.interactionEvent.getSelectedItem() + " Region '" + (SketchletEditor.editorPanel.currentPage.regions.regions.size() - SketchletEditor.editorPanel.currentPage.regions.regions.indexOf(mp.region)) + "' do:");
            setInteractionEvent("Start timer", timer.name, "", comments);
        } else if (mp.type == 3) {
            comments.add("On '" + SketchletEditor.editorPanel.currentPage.title + "' entry:");
            setSketchEntryEvent("Start timer", timer.name, "", comments);
        } else if (mp.type == 5) {
            comments.add("On '" + SketchletEditor.editorPanel.currentPage.title + "' variable update:");
            setSketchVariableEvent(mp.variableUpdateEventPage.variableCombo.getSelectedItem().toString(), mp.variableUpdateEventPage.operatorCombo.getSelectedItem().toString(), mp.variableUpdateEventPage.value.getText(), "Start timer", timer.name, "", comments);
        } else if (mp.type == 6) {
            comments.add("On '" + SketchletEditor.editorPanel.currentPage.title + "' keyboard event:");
            setSketchKeyboardEvent(mp.keyboardEventPage.ctrlBox.isSelected(), mp.keyboardEventPage.altBox.isSelected(), mp.keyboardEventPage.shiftBox.isSelected(), mp.keyboardEventPage.variableKeys.getSelectedItem().toString(), mp.keyboardEventPage.variableEvents.getSelectedItem().toString(), "Start timer", timer.name, "", comments);
        } else if (mp.type == 4) {
            comments.add("On '" + SketchletEditor.editorPanel.currentPage.title + "' exit:");
            setSketchExitEvent("Start timer", timer.name, "", comments);
        }
    }
    ...
    public void processMouseEvent(Vector<String> comments) {
        try {
            ...
            ActiveRegionsExtraPanel.reload(SketchletEditor.editorPanel.currentPage.regions.getLastSelectedRegion());
            if (ActiveRegionsExtraPanel.regionsAndActions != null) {
                net.sf.sketchlet.designer.ui.region.ActiveRegionPanel panel = (net.sf.sketchlet.designer.ui.region.ActiveRegionPanel) ActiveRegionsExtraPanel.regionsAndActions.tabs.getSelectedComponent();

                if (panel != null) {
                    panel.tabs.setSelectedIndex(net.sf.sketchlet.designer.ui.region.ActiveRegionPanel.indexEvents);
                    panel.tabsRegionEvents.setSelectedIndex(net.sf.sketchlet.designer.ui.region.ActiveRegionPanel.indexMouseEvents);
                }
            }
        } catch (Exception e) {
            e.printStackTrace();
    ...
    public void processInteractionEvent(Vector<String> comments) {
        try {
            ...
            if (ActiveRegionsExtraPanel.regionsAndActions != null) {
                ...
                if (panel != null) {
                    panel.tabs.setSelectedIndex(net.sf.sketchlet.designer.ui.region.ActiveRegionPanel.indexEvents);
                    panel.tabsRegionEvents.setSelectedIndex(net.sf.sketchlet.designer.ui.region.ActiveRegionPanel.indexOverlap);
                }
            }
        } catch (Exception e) {
            e.printStackTrace();
    ...
    public void setRegionProperty(int regionIndex, String property, int p, String value, Vector<String> comments) {
        try {
            ActiveRegion region = SketchletEditor.editorPanel.currentPage.regions.regions.elementAt(SketchletEditor.editorPanel.currentPage.regions.regions.size() - regionIndex);
            if (p == 0) {
    ...
    public void setSketchEntryEvent(String action, String param1, String param2, Vector<String> comments) {
        Object actions[][] = SketchletEditor.editorPanel.currentPage.onEntryMacro.actions;
        for (int i = 0; i < actions.length; i++) {
            if (actions[i][0].toString().isEmpty()) {
                ...
                if (SketchletEditor.editorPanel.statePanel != null && SketchletEditor.editorPanel.statePanel.pOnEntry != null) {
                    SketchletEditor.editorPanel.statePanel.pOnEntry.reload();
                }
                break;
            }
        }
    ...
    public void setSketchExitEvent(String action, String param1, String param2, Vector<String> comments) {
        Object actions[][] = SketchletEditor.editorPanel.currentPage.onExitMacro.actions;
        for (int i = 0; i < actions.length; i++) {
            if (actions[i][0].toString().length() == 0) {
                ...
                if (SketchletEditor.editorPanel.statePanel != null && SketchletEditor.editorPanel.statePanel.pOnExit != null) {
                    SketchletEditor.editorPanel.statePanel.pOnExit.reload();
                }
                break;
            }
        }
