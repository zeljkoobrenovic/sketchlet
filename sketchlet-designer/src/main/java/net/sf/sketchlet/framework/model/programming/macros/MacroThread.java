...
public class MacroThread implements Runnable, RunInterface {
    ...
    public MacroThread(String strCommands, String strVarPrefix, String strVarPostfix) {
        ...
        for (String cmd : commands) {
            ...
            if (!action.isEmpty() && i < this.macro.getActions().length) {
                this.macro.getActions()[i][0] = action;
                this.macro.getActions()[i][1] = param1;
                this.macro.getActions()[i][2] = param2;
                i++;
            }
        }
    ...
    public int executeSequence(int startIndex) {
        ...
        int i;
        for (i = startIndex; i < macro.getActions().length && !stopped; i++) {
            if (stopped) {
                return i;
            }
            ...
            String action = processForArgs(args, (String) macro.getActions()[i][0]);
            String param1 = processForArgs(args, (String) macro.getActions()[i][1]);
            String param2 = processForArgs(args, (String) macro.getActions()[i][2]);
            ...
            if (action.equalsIgnoreCase("end") || action.equalsIgnoreCase("")) {
                ...
                if (result == null || !(result instanceof Double) || ((Double) result).doubleValue() == 0.0) {
                    ...
                    while (!stopped && i < macro.getActions().length) {
                        i++;
                        action = (String) macro.getActions()[i][0];

                        if (l == macro.getLevels()[i] && (action.equalsIgnoreCase("end") || action.isEmpty())) {
                            break;
                        }
                    }
                } else {
                ...
                boolean bContinue = page == null ? false : Commands.execute(source == null ? this : source, action, param1, param2, page.getActiveTimers(), page.getActiveMacros(), strVarPrefix, strVarPostfix, null);
                if (!bContinue) {
                    stopped = true;
                    break;
                }
    ...
    public void run() {
        ...
        int nActive = 0;
        for (int i = 0; i < macro.getActions().length && !stopped; i++) {
        ...
        if ((PlaybackFrame.playbackFrame != null && progressMonitor != null)
                || (SketchletEditor.getInstance() != null && SketchletEditor.getInstance().getInternalPlaybackPanel() != null)) {
            for (int r = 0; !stopped && (r < macro.getRepeat() || macro.getRepeat() == 0); r++) {
                executeSequence(0);
            }
        } else {
            for (int r = 0; !stopped && (r < macro.getRepeat() || macro.getRepeat() == 0); r++) {
