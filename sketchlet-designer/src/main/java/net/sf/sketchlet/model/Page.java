...
public class Page implements PropertiesInterface {
    ...
    private boolean bLayerRegions = true;

    public boolean isRegionsLayerActive() {
        return this.isbLayerRegions();
    }
                    ...
                    if (getImageUpdated()[i] || bSaveImageThumb) {
                        try {
                            ...
                            if (i == 0 && getImages() != null && getImages()[0] != null) {
                                ...
                                getRegions().draw(g2, SketchletEditor.getInstance(), SketchletEditor.getInstance().getMode(), false, false, 1.0f);
                                g2.dispose();
    ...
    public void saveSketchFile(PrintWriter outSketch) {
        try {
            ...
            if (getLocalVariables().size() > 0) {
                ...
                for (LocalVariable localVariable : this.getLocalVariables()) {
    ...
    public void deactivate(boolean bPlayback) {
        try {
            ...
            if (bPlayback && getOnExitMacro() != null) {
    ...
    public void activate(boolean bPlayback) {
        ...
        if (bPlayback) {
            ...
            getRegions().refreshFromVariables();
        }
    ...
    public boolean isbLayerRegions() {
        return bLayerRegions;
    }

    public void setbLayerRegions(boolean bLayerRegions) {
        this.bLayerRegions = bLayerRegions;
    }
}
