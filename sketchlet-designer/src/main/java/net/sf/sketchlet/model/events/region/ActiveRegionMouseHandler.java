...
public class ActiveRegionMouseHandler {
    ...
    private int playback_startX;
    private int playback_startY;
    private double startAngle = 0.0;
    private double playback_startAngle;
    ...
    public void mouseMoved(MouseEvent e, double scale, JFrame frame, boolean bPlayback) {
        if (!region.parent.getPage().isbLayerRegions()) {
            return;
        }
        int marginX = bPlayback ? region.parent.getOffset_x() : SketchletEditor.getInstance().getMarginX();
        int marginY = bPlayback ? region.parent.getOffset_y() : SketchletEditor.getInstance().getMarginY();
    ...
    public void mousePressed(int x, int y, int modifiers, long when, MouseEvent e, JFrame frame, boolean bPlayback) {
        ...
        setPressedY(y);
        if (bPlayback) {
            setPlayback_startX(x);
            setPlayback_startY(y);
        } else {
            setStartX(x);
            setStartY(y);
        }
        ...
        if (bPlayback) {
            if ((modifiers & InputEvent.BUTTON1_MASK) == InputEvent.BUTTON1_MASK) {
                ...
                if (x == xCenter) {
                    setPlayback_startAngle(y > yCenter ? Math.PI / 2 : -Math.PI / 2);
                } else {
                    setPlayback_startAngle(Math.atan((double) (y - yCenter) / (x - xCenter)));
                }

                setPlayback_startAngle(getPlayback_startAngle() - region.playback_rotation);

                if (x < xCenter) {
                    setPlayback_startAngle(getPlayback_startAngle() + Math.PI);
                }

                if (getPlayback_startAngle() < 0) {
                    setPlayback_startAngle(getPlayback_startAngle() + 2 * Math.PI);
                }
            }
    ...
    public void mouseReleased(int x, int y, int modifiers, long when, MouseEvent e, JFrame frame, boolean bPlayback) {
        ...
        region.getMotionHandler().processLimits("speed", 0.0, 0.0, 0.0, true);
        if (bPlayback) {
    ...
    public synchronized void mouseDragged(int x, int y, int modifiers, long when, double scale, MouseEvent e, JFrame frame, boolean bPlayback) {
        ...
        if (bPlayback && (!region.strWidget.isEmpty())) {
            ...
            if (getSelectedCorner() != ROTATE && getSelectedCorner() != TRAJECTORY2_POINT && getSelectedCorner() != CENTER_ROTATION) {
                ...
                if (!region.isWithinLimits(false) || region.getInteractionHandler().intersectsWithSolids(bPlayback)) {
                ...
                if (!bPlayback) {
                    if (region.parent != null && region.parent.getSelectedRegions() != null) {
                        for (ActiveRegion as : region.parent.getSelectedRegions()) {
                            if (as != region) {
                ...
                if (region.horizontalAlignment.equalsIgnoreCase("center")) {
                    region.getMotionHandler().processLimits("position x", region.x1 + w / 2, w / 2, w / 2, true);
                } else if (region.horizontalAlignment.equalsIgnoreCase("right")) {
                    region.getMotionHandler().processLimits("position x", region.x1 + w, w, 0, true);
                } else {
                    region.getMotionHandler().processLimits("position x", region.x1, 0, w, true);
                }
                if (region.verticalAlignment.equalsIgnoreCase("center")) {
                    region.getMotionHandler().processLimits("position y", region.y1 + h / 2, h / 2, h / 2, true);
                } else if (region.verticalAlignment.equalsIgnoreCase("right")) {
                    region.getMotionHandler().processLimits("position y", region.y1 + h, h, 0, true);
                } else {
                    region.getMotionHandler().processLimits("position y", region.y1, 0, h, true);
                }
                ...
                if (region.inTrajectoryMode) {
                    ...
                    if (!region.trajectory1.trim().equals("")) {
                        if (region.stickToTrajectoryEnabled) {
                            ...
                            region.getMotionHandler().processLimits("trajectory position", region.getRenderer().getTrajectoryDrawingLayer().trajectoryPositionFromPoint, 0.0, 1.0, 0.0, 0.0, true);
                            region.getMotionHandler().processLimits("trajectory position 2", region.getRenderer().getTrajectoryDrawingLayer().trajectoryPositionFromPoint2, 0.0, 1.0, 0.0, 0.0, true);
                        }
                        if (bPlayback) {
                            region.resetFromPlayback();
                            region.setDataFromPlayback();
    ...
    public void rotate(int x, int y, boolean bPlayback) {
        ...
        region.rotation = InteractionSpace.toRadians(region.getMotionHandler().processLimits("rotation", InteractionSpace.toPhysicalAngle(region.rotation), 0.0, 0, true));
        if (bPlayback) {
            region.resetFromPlayback();
            region.setDataFromPlayback();
        }
    ...
    public int getPlayback_startX() {
        return playback_startX;
    }

    public void setPlayback_startX(int playback_startX) {
        this.playback_startX = playback_startX;
    }

    public int getPlayback_startY() {
        return playback_startY;
    }

    public void setPlayback_startY(int playback_startY) {
        this.playback_startY = playback_startY;
    }
    ...
    public double getPlayback_startAngle() {
        return playback_startAngle;
    }

    public void setPlayback_startAngle(double playback_startAngle) {
        this.playback_startAngle = playback_startAngle;
    }
