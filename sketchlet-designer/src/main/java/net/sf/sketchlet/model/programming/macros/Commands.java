...
public class Commands {
    ...
    public static boolean execute(final Object source, final String action, String param1, String param2, List<TimerThread> activeTimers, List<RunInterface> activeMacros, String strVarPrefix, String strVarPostfix, JFrame frame) {
        ...
        if (action.startsWith("Variable") && param2.equals("?")) {
            ...
            if (PlaybackFrame.playbackFrame != null || SketchletEditor.getInstance().getInternalPlaybackPanel() != null) {
                if (frame == null) {
        ...
        if (action.equalsIgnoreCase("Pause") || action.equalsIgnoreCase("Pause (seconds)")) {
            ...
            synchronized (activeTimers) {
                ...
                for (int i = activeTimers.size() - 1; i >= 0; i--) {
                    TimerThread tt = activeTimers.get(i);
                    if ((param2.trim().isEmpty() || tt.getParameters().trim().equals(param2.trim())) && tt.getTimer().getName().trim().equalsIgnoreCase(param1.trim())) {
                        tt.continueExecution();
                        bContinued = true;
                    }
                }
                if (!bContinued) {
                    if (Timers.getGlobalTimers() != null) {
                        TimerThread tt = Timers.getGlobalTimers().startTimerThread(param1, param2, activeTimers);
                    }
                }
            }
            ...
            synchronized (activeTimers) {
                for (int i = activeTimers.size() - 1; i >= 0; i--) {
                    TimerThread tt = activeTimers.get(i);
                    if (tt.getParameters().trim().equals(param2.trim()) && tt.getTimer().getName().trim().equalsIgnoreCase(param1.trim()) && !tt.isPaused()) {
                        tt.stop();
                        activeTimers.remove(i);
                    }
                }
            ...
            if (PlaybackFrame.playbackFrame != null && PlaybackFrame.playbackFrame[0] != null && PlaybackFrame.playbackFrame[0].isVisible()) {
                ...
                for (int i = 0; i < PlaybackFrame.playbackFrame.length; i++) {
                    PlaybackFrame.playbackFrame[i].setTitle(PlaybackPanel.currentPage.getTitle());
                ...
                SketchletEditor.getInstance().selectSketch(param1.trim());
                bContinue = param1.trim().equals("");
            }
            ...
            if (Macros.globalMacros != null) {
                if (param1.trim().startsWith("{") && param2.trim().endsWith("}")) {
                    ...
                    if (ScreenScripts.getPublicScriptRunner() != null) {
                        ScreenScripts.getPublicScriptRunner().executeScreenAction(param1.substring(7));
                    }
                    ...
                    try {
                        ...
                        for (int i = 0; i < Workspace.getConsolePane().getIoServicesHandler().getTabbedPane().getTabCount(); i++) {
                            service = (ProcessConsolePanel) Workspace.getConsolePane().getIoServicesHandler().getTabbedPane().getComponentAt(i);
            ...
            if (Macros.globalMacros != null && activeMacros != null) {
                for (int i = activeMacros.size() - 1; i >= 0; i--) {
                    RunInterface ri = activeMacros.get(i);
        ...
        if (param1.startsWith("[") && param1.endsWith("]")) {
            ...
            try {
                ...
                if (strID.equalsIgnoreCase("sketch") || strID.equalsIgnoreCase("page")) {
                    ...
                    properties = page.getRegions().getRegionByName(strID);
                }
                if (properties != null) {
