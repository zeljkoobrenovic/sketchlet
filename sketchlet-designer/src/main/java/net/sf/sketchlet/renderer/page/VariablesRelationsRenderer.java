...
public class VariablesRelationsRenderer {
    ...
    public static void drawVariables(ActiveRegions regions, Graphics2D g2, boolean bPlayback) {
        ...
        Page page = bPlayback ? PlaybackPanel.currentPage : SketchletEditor.getInstance().getCurrentPage();
        ...
        while (info.hasMoreElements()) {
            ...
            vi.setTextWidth((int) font.getStringBounds(strVarText, frc).getWidth() + 4);
        ...
        for (String strTimer : timers) {
            ...
            if (bPlayback) {
                timer = getTimerThread(PlaybackPanel.currentPage.getActiveTimers(), strTimer);
            } else {
...
class DrawVariableInfo {
    ...
    public void setRegions(Hashtable<ActiveRegion, String> regions) {
