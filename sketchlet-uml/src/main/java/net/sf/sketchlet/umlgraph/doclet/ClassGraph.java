 ...
 *
 */
package net.sf.sketchlet.umlgraph.doclet;
...
/**
 * Class graph generation engine @depend - - - StringUtil @depend - - - Options
 * @composed - - * ClassInfo @has - - - OptionProvider
 *
...
class ClassGraph {
    ...
    /**
     * Create a new ClassGraph. <p>The packages passed as an argument are the
     * ones specified on the command line.</p> <p>Local URLs will be generated
    ...
    private String qualifiedName(Options opt, String r) {
        if (!opt.showQualified) {
            ...
            for (;;) {
                 ...
                 * Change all occurences of "p1.p2.myClass<S extends
                 * dummy.Otherclass>" into "myClass<S extends Otherclass>"
                 */
                int start = dotpos;
    ...
    /**
     * Escape &lt;, &gt;, and &amp; characters in the string with the
    ...
    /**
     * Convert &lt; and &gt; characters in the string to the respective
     ...
     *
     * @param str the
     * <code>String</code> to be wrapped.
     * @return the wrapped
     * <code>String</code>.
     */
    ...
    /*
     * The following two methods look similar, but can't be refactored into one,
     * because their common interface, ExecutableMemberDoc, doesn't support
     * returnType for ctors.
     */
    /**
    ...
    /**
     * Return as a string the stereotypes associated with c terminated by the
     * escape character term
     */
    ...
    /**
     * Return true if the class name is associated to an hidden class or matches
     * a hide expression
     */
    ...
    /**
     * Prints the class if needed. <p> A class is a rootClass if it's included
     * among the classes returned by RootDoc.classes(), this information is used
     * to properly compute relative links in diagrams for UMLDoc
     */
    public String printClass(ClassDoc c, boolean rootClass) {
        ...
        if (toPrint && !hidden(c) && (!c.isEnum() || opt.showEnumerations)) {
            ...
            if (opt.showComment) {
                tableLine(Align.CENTER, htmlNewline(escape(c.commentText())), opt, Font.CLASS);
                if (opt.namespace != null && !opt.namespace.isEmpty()) {
                    String packageName = opt.namespace.trim();
                ...
                tableLine(Align.CENTER, packageName, opt, Font.PACKAGE);
            } else if (opt.namespace != null && !opt.namespace.isEmpty()) {
                String packageName = opt.namespace.trim();
                String cn = className;
                tableLine(Align.CENTER, escape(cn), opt, font);
                tableLine(Align.CENTER, packageName, opt, Font.PACKAGE);
            ...
            /*
             * Warning: The boolean expressions guarding innerTableStart() in
             * this block, should match those in the code block above marked:
             * "Calculate the number of innerTable rows we will emmit"
             */
            if (showMembers) {
                if (opt.showAttributes) {
                ...
                if (!c.isEnum() && (opt.showConstructors || opt.showOperations)) {
                    ...
                    if (!printedLines) // if there are no operations nor constructors,
    ...
    /**
     * Return the full name of a relation's node. This may involve appending the
     * port :p for the standard nodes whose outline is rendered through an inner
     * table.
     */
    ...
    /**
     * Return the full name of a relation's node c. This may involve appending
     * the port :p for the standard nodes whose outline is rendered through an
     * inner table.
     *
    ...
    /**
     * Prints associations recovered from the fields of a class. An association
     * is inferred only if another relation between the two classes is not
    ...
    /**
     * Prints dependencies recovered from the methods of a class. A dependency
     * is inferred only if another relation between the two classes is not
     * already in the graph.
     *
     * @param classes
    ...
    /**
     * Returns all program element docs that have a visibility greater or equal
    ...
    /**
     * Returns the appropriate URL "root" for a given class name. The root will
     * be used as the prefix of the URL used to link the class in the final
     * diagram to the associated JavaDoc page.
     */
    ...
    /**
     * Wraps the text with the appropriate font according to the specified font
     * type
     *
     * @param opt
     * @param text
    ...
    /**
     * Wraps the text with the appropriate font tags when the font name and size
     * are not void
     *
